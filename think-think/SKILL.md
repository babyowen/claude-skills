---
name: think-think
description: 深度思考辅助工具，采用多agent协作模式帮助用户澄清目标、收集背景、设计方案、批判审视，最终输出结构化思考报告。使用场景：用户需要进行深度思考、问题分析、方案设计、决策支持时。触发词包括 /think-think、深度思考、帮我思考、深入分析等。
user-invocable: true
---

# Think-Think - 深度思考辅助工具

## 概述

Think-Think 是一个结构化的深度思考辅助工具，通过多Agent协作帮助用户：
...
### 触发方式
```
/think-think 我应该如何提升团队的技术能力？
```

### 交互示例
```
🎯 Think-Think 深度思考助手已启动
...
| 触发条件 | 处置 |
|---|---|
| `@agents/*.md` 加载失败（文件缺失/损坏） | 主 agent 临时承担该角色，按各 Phase 输出格式执行，并标注降级 |
| 用户中途退出 / 长时间无响应 | 保存当前状态、输出已完成阶段成果，不强迫继续 |
| 背景信息严重不足且无法补充 | 标注「信息缺口」，基于已有信息推进或暂停等用户补充，**不编造背景** |
| WebSearch / 联网工具不可用 | 改用本地文件 + 用户提供信息，标注来源限制 |
| 阶段间反复横跳（连续「返回修正」） | 提示聚焦，最多允许 2 次回跳后建议基于当前最优推进 |
| 无法生成多候选方案 | 至少保证 1 个主方案 + 风险分析，不强行凑数 |

## 反例（不要做）

- ❌ 问卷式轰炸——一次抛多个问题；应一次问最关键的 1-2 个
- ❌ 强迫走完全部 5 Phase——用户选「跳过 / 输出当前结果」时尊重，不纠缠
- ❌ 跳过 Phase 1 澄清直接出方案——目标未明确时方案必跑偏
- ❌ 编造背景信息 / 数据来源——信息缺口要标注，不能虚构
- ❌ 无限循环修正——适时建议推进，避免在单一阶段过度停留

---

*Think-Think v1.0 - 让思考更有深度*

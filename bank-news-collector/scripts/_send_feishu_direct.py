#!/usr/bin/env python3
"""Send Feishu message directly via Lark OpenAPI (bypassing lark-cli)."""
import json
import os
import sys
...
def get_tenant_token(app_id, app_secret):
    ...
    if result.get("code") != 0:
        print(f"Failed to get tenant token: {result}", file=sys.stderr)
        sys.exit(1)
...
def main():
    config = get_config()
    app_id = config.get("app_id") or config.get("appId")
    app_secret = config.get("app_secret") or config.get("appSecret")

    if not app_id or not app_secret:
        print(f"ERROR: Missing app_id/app_secret in config. Keys found: {list(config.keys())}", file=sys.stderr)
        sys.exit(1)
    ...
    if result.get("code") == 0:
        ...
        # Only output the message_id, not the full response
        print(f"Message sent successfully. message_id={msg_id}")
    else:
        print(f"Failed to send message: code={result.get('code')}, msg={result.get('msg')}", file=sys.stderr)
        sys.exit(1)

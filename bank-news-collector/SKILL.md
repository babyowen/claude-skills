...
| 模式 | 触发条件 | Agent 行为 | 用户交互 |
|------|----------|------------|----------|
| **定时自动** | 外部调度器（crontab/launchd）触发 | 静默执行步骤1-9，0输出到控制台 | 无 |
| **手动全量** | 用户说"采集今天的银行新闻" | 完整执行步骤1-9，仅步骤8输出 | 无（自动化） |
| **只读报告** | 用户说"看看今天有什么新闻" | 跳过采集，直接执行步骤8 | 无 |
| **单站补采** | 用户指定"只抓东方财富" | 跳过步骤2a的全站抓取，只执行指定站点 | 需确认 |

**不适用此skill的场景：**
- 查询单条具体新闻的详细内容 → 直接 web_fetch
- 需要历史跨天汇总 → 使用 bank-daily-report skill
- 只需要新闻标题列表、不需要筛选总结 → 直接运行 `fetch_all_candidates.py`

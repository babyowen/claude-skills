...
description: 为银行从业者自动采集和筛选与银行经营高度相关的新闻。当用户提到银行新闻、同业动态、监管变化、需要从财经网站批量筛选新闻并按日期归档、过滤当天已收录新闻、提炼对银行经营有启发的内容时，务必使用此技能。此技能专为定时任务设计，支持完全自动化的无人值守运行，即使没有明确用户指令，只要涉及银行新闻的定时采集，都应使用此技能。
---
...
步骤6：第二轮筛选 - Agent 严筛 + 生成总结（静默）
   ├─ 逐条用 Agent 自身 LLM 判断：
   │  ├─ 是否今天发布？（严格约束）
   │  │  ├─ 从正文中识别发布日期，日期可能出现在以下位置：
   │  │  │  ├─ 标题与正文之间（如 "第一财经 _03-29 10:27_"）
   │  │  │  ├─ 作者/编辑署名行附近
   │  │  │  ├─ 正文开头第一段
   │  │  │  └─ 文末或评论区附近
   │  │  ├─ 日期格式可能是：
   │  │  │  ├─ 绝对日期：03-29、03-29 10:27、2026/03/29、2026年3月29日
   │  │  │  ├─ 相对日期：昨天、2小时前、刚刚（需结合当前时间推断）
   │  │  │  └─ 混合格式：第一财经 _03-27 23:50_
   │  │  ├─ 判断规则：
   │  │  │  ├─ 日期明确=今天 → 继续判断相关性
   │  │  │  ├─ 日期明确≠今天 → 剔除
   │  │  │  └─ 找不到日期 → 无法确认，剔除
   │  │  └─ 将识别到的日期填入 published_at 字段
   │  └─ 是否与银行经营高度相关？（严出：确实有影响、风险提示或启发）
   ├─ 为通过的新闻生成约500字总结（站在银行经营视角提炼影响、风险、启发）
   ├─ 添加 why_relevant 字段（一句话具体说明相关性）
   ├─ 保存通过的结果到 data/second_round_filtered.json
   └─ 格式：{"title", "url", "summary", "published_at", "source", "why_relevant", "collected_at"}
